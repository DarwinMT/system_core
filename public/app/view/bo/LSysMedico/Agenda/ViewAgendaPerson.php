				...
				<div class="row" ng-hide=" tipo_calendar!='ODONT' ">
					...
					<div class="row">
						...
						<div class="col-md-6 col-xs-12 text-center btn-group">
							...
							<button type="button" class="btn btn-primary btn-sm" title="Generar Proforma" ng-click="genera_proforma()">
								<i class="glyphicon glyphicon-cog"></i> Proforma
							</button>

							<button type="button" class="btn btn-primary btn-sm" title="Historial Prefactura Y Pagos" ng-click="historial_prefactura_pagos()">
								<i class="glyphicon glyphicon-cog"></i> Historial
							</button>
...
<div class="modal fade" id="proforma_factura" tabindex="-1" role="dialog" data-backdrop="static" data-keyboard="false" >
  <div class="modal-dialog modal-lg" role="document">
	<div class="modal-content">
	  <div class="modal-header bg-primary">
		<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
		<h4 class="modal-title">Proforma/Prefactura</h4>
	  </div>
	  <div class="modal-body">
		<div class="row">
			<div class="col-md-12 col-xs-12">
				<table class="table table-bordered table-condensed ">
					...
					<tbody>
						<tr ng-repeat="prf in lista_tratamientos_aplicados">
							<td>{{$index+1}}</td>
							<td>{{prf.Diente}}</td>
							<td>{{prf.DescripcionTratamineto}}</td>
							<td>{{prf.Cantidad}}</td>
							<td><input type="text" ng-keyup="calcula_totales();" class="form-control input-sm" ng-model="prf.PrecioU" /></td>
							<td><input type="text" ng-keyup="calcula_totales();" class="form-control input-sm" ng-model="prf.Descuento" /></td>
							<td>{{prf.Total}}</td>
						</tr>
						<tr>
							
							<td  colspan="4" >
								<textarea class="form-control input-sm" placeholder="Descripcion " ng-model="descripcion_prefactura" rows="1" cols="100"></textarea>
							</td>
							<th></th>
								...
								<div class="row">
									<div class="col-xs-12">
										<div class="input-group">
											...
											<input type="text" class="form-control input-sm" ng-keyup="valida_pago();"  placeholder="Valor A Pagar"  name="dineropago" id="dineropago" ng-model="dineropago" />
										</div>
									</div>
								</div>
						...
						<tr>
							...
							<td ><input type="text" ng-keyup="calcula_totales();" class="form-control input-sm" ng-model="val_iva" /></td>
							<th >{{iva}}</th>
						</tr>
						<tr>
							<td ></td>
							<th class="text-right">Total</th>
							<th >{{total_prefactura}}</th>
						</tr>
					</tbody>
				</table>
			</div>
	  ...
	  <div class="modal-footer">
		<button type="button" class="btn btn-default btn-sm" data-dismiss="modal"><i class="glyphicon glyphicon-ban-circle"></i> Cancelar</button>
		<button type="button" class="btn btn-success btn-sm" ng-click="save_prefactura_pago();" ><i class="glyphicon glyphicon-cog"></i> Generar Prefactura</button>
	  </div>
	</div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->

<div class="modal fade" id="list_prefactura_pago"  tabindex="-1" role="dialog" data-backdrop="static" data-keyboard="false"  >
  <div class="modal-dialog modal-lg" style="width:90%;" role="document">
    <div class="modal-content">
      <div class="modal-header bg-primary">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title">Lista Prefacturas</h4>
      </div>
      <div class="modal-body">
        <div class="row">
			<div class="col-md-6 col-xs-12">
				<table class="table table-bordered table-condensed">
					<thead class="bg-primary">
						<tr>
							<th class="text-center" colspan="7">Prefactura Pagadas</th>
						</tr>
						<tr>
							<th></th>
							<th>Descripción</th>
					...
					<tbody>
						<tr ng-repeat=" pag in prefacturapagada">
							<td>{{$index+1}}</td>
							<td>{{pag.descripcion}}</td>
							<td>{{pag.subtotal}}</td>
							<td>{{pag.descuento}}</td>
							<td>{{pag.total}}</td>
							<td ng-hide="pag.estado=='1' ||  pag.estado=='2' " class="bg-danger">PENDIENTE</td>
							<td ng-hide="pag.estado=='0' ||  pag.estado=='2' " class="bg-warning">ABONADO</td>
							<td ng-hide="pag.estado=='0' ||  pag.estado=='1' " class="bg-success">PAGADA</td>
							<td>{{(pag.anulado=='0')? "VALIDA":"ANULADA"}}</td>
						</tr>
					</tbody>
				</table>
			</div>
			<div class="col-md-6 col-xs-12">
				<table class="table table-bordered table-condensed">
					<thead class="bg-primary">
						<tr>
							<th class="text-center" colspan="8">Prefactura Inpagadas</th>
						</tr>
						<tr>
							<th></th>
							<th>Descripción</th>
					...
					<tbody>
						<tr ng-repeat=" impag in prefacturainpagada">
							<td>{{$index+1}}</td>
							<td>{{impag.descripcion}}</td>
							<td>{{impag.subtotal}}</td>
							<td>{{impag.descuento}}</td>
							<td>{{impag.total}}</td>
							<td ng-hide="impag.estado=='1' ||  impag.estado=='2' " class="bg-danger">PENDIENTE</td>
							<td ng-hide="impag.estado=='0' ||  impag.estado=='2' " class="bg-warning">ABONADO</td>
							<td ng-hide="impag.estado=='0' ||  impag.estado=='1' " class="bg-success">PAGADA</td>
							<td>{{(impag.anulado=='0')? "VALIDA":"ANULADA"}}</td>
							<td>
								<button class="btn btn-primary btn-sm" title="Pagar Prefactura" ng-click="pagar_prefactura(impag)" >
